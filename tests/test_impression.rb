...
class ImpressionTest < Test::Unit::TestCase
  ...
  include VWO::Enums
  EVENT_ARCH_QUERY_PARAMS = %w[a en eTime random env p]
  ...
  def check_all_properties_present(properties, expected_properties)
    expected_properties.each do |field|
      return false unless properties.key?(field.to_sym)
    end
    true
  end

  def check_all_properties_present_and_their_types(properties, expected_properties)
    expected_properties.each do |key, _value|
      return false if !(properties.key? key) || (properties[key]).class != (expected_properties[key]).class

      if (properties[key]).is_a?(Hash)
